...
$('#userName').blur(function () {
    ...
    $.get(
        "/beadhouse/register/administratorInfo",
        {adminName: name},
        function (data) {
            if (data == true) {
                //alert("用户名已存在");
...
$('#country').mouseleave(function () {
    ...
    $.get(
        ...
        function (data) {
            ...
            $("#beadhouseselect").html("");
            if (keys != null && keys.length != 0) {
                for (var index in keys) {
                    var newNode = "<option value='" + keys[index] + "'>" + data[keys[index]] + "</option>";
                    $("#beadhouseselect").append(newNode);
                }
                return;
            }
            var node = "<option id='chooseBeadHouse' value='-1'>请选择养老院</option>";
            $("#beadhouseselect").append(node);

        }
    );
});
function canBeRegister() {
    if ($("#beadhouseselect").val() == -1) {
        alert("请选择养老院");
        return false;
    }
    if (usernameCanBeUsed && passwordCanBeUsed && emailAddressCanBeUsed && telephoneNumberCanBeUsed) {
        ...
        $.post(
            "/beadhouse/register",
            {
                ...
                beadhouseId: $("#beadhouseselect").val(),
                realName: $("#realName").val()
            },
            function (data) {
                if (data == true) {
                    alert("注册成功");
                    window.location.href = "/beadhouse";
                    return true;
                } else {
                    alert("信息有误");
                    return false;
