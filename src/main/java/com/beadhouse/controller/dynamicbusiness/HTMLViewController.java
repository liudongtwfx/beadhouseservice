package main.java.com.beadhouse.controller.dynamicbusiness;
...
public class HTMLViewController {
    ...
    public String submitChanges(HttpServletRequest request, Model model) throws IOException {
        ...
        DomInfo domInfo = new DomInfo(file.getAbsolutePath());
        domInfo.createNodesInfo();
        String callbackinfo = domInfo.createHTMLTree(request);
        if (callbackinfo.equals("seccess")) {
            model.addAttribute("callbackclass", "text-seccess");
            model.addAttribute("callbackinfo", "已成功更改视图文件");
        } else {
            model.addAttribute("callbackclass", "text-danger");
            model.addAttribute("callbackinfo", callbackinfo);
        }
    ...
    public String submitAdd(HtmlView view, BindingResult result, HttpServletRequest request, Model model) {
        try {
